...
## @deftypefnx {} {[@dots{}] =} mysqp (@var{x0}, @var{phi}, @var{g})
## @deftypefnx {} {[@dots{}] =} mysqp (@var{x0}, @var{phi}, @var{g}, @var{h})
## @deftypefnx {} {[@dots{}] =} mysqp (@var{x0}, @var{phi}, @var{g}, @var{h}, @var{lb}, @var{ub})
## @deftypefnx {} {[@dots{}] =} mysqp (@var{x0}, @var{phi}, @var{g}, @var{h}, @var{lb}, @var{ub}, @var{maxiter})
## @deftypefnx {} {[@dots{}] =} mysqp (@var{x0}, @var{phi}, @var{g}, @var{h}, @var{lb}, @var{ub}, @var{maxiter}, @var{tolerance})
...
## @end table
##
## An example of calling @code{mysqp}:
##
## @example
## function r = g (x)
##   r = [ sumsq(x)-10;
...
##
## [x, obj, info, iter, nf, lambda] = mysqp (x0, @@phi, @@g, [])
##
## x =
##
##   -1.71714
...
function [x, obj, info, iter, nf, lambda] = mysqp (x0, objf, cef, cif, lb, ub, maxiter, tolerance)
  ...
  if (! isvector (x0))
    error ("mysqp: X0 must be a vector");
  endif
  if (rows (x0) == 1)
    x0 = x0';
  endif
  ...
  if (iscell (objf))
    switch (numel (objf))
      ...
      case 3
        ...
        have_hess = 1;
      otherwise
        error ("mysqp: invalid objective function specification");
    endswitch
  else
    ...
    if (iscell (cef))
      switch (numel (cef))
        ...
        case 2
          ...
          ce_grd = cef{2};
        otherwise
          error ("mysqp: invalid equality constraint function specification");
      endswitch
    elseif (! isempty (cef))
  ...
  if (nargin > 3)
    ...
    if (nargin < 5 || (nargin > 5 && isempty (lb) && isempty (ub)))
      ...
      if (iscell (cif))
        switch (length (cif))
          ...
          case 2
            ...
            ci_grd = cif{2};
          otherwise
           error ("mysqp: invalid inequality constraint function specification");
        endswitch
      elseif (! isempty (cif))
      ...
      elseif (isempty (lb))
        ...
        error ("mysqp: invalid lower bound");
      endif
      ...
      elseif (isempty (ub))
        ...
        error ("mysqp: invalid upper bound");
      endif

      if (any (tmp_lb > tmp_ub))
        error ("mysqp: upper bound smaller than lower bound");
      endif
  ...
  if (nargin > 6 && ! isempty (maxiter))
    if (isscalar (maxiter) && maxiter > 0 && fix (maxiter) == maxiter)
      ...
      error ("mysqp: invalid number of maximum iterations");
    endif
  endif
  ...
  if (nargin > 7 && ! isempty (tolerance))
    if (isscalar (tolerance) && tolerance > 0)
      ...
      error ("mysqp: invalid value for TOLERANCE");
    endif
  endif
  ...
  while (++iter < iter_max)
    ...
    [p, obj_qp, INFO, lambda] = qp (x, B, c, F, g, [], [], d, C,
                                    Inf (size (d)), struct ("TolX", tol,"MaxIter",500));
